...
quality_levels = list()
for blueprint_index in range(len(blueprints)):
    ...
    while status_queue:
        ...
        status.run_minute()
        if status.get_minute_count() in best_geode_count.keys() and status.get_geode_count() < best_geode_count[status.get_minute_count()]:
            # There is a better path. Skip this one.
            continue
        else:
            best_geode_count[status.get_minute_count()] = status.get_geode_count()

        if status.get_minute_count() < 24:
    ...
    quality_levels.append((blueprint_index + 1) * max_geode_count)

print(f"Total quality level: {sum(quality_levels)}")
